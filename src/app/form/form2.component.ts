...
export class Form2Component implements OnInit {
  ...
  academicoRedondeado: number = 0;
  modoImpresion: Boolean[] = [true, false];
  ...
  validezFormulario_Acta_INSTITUCION_EDUCATIVA = new Array(11).fill(false);
  ...
  async ngOnInit() {
    ...
    await this.sendRequestWithRetry(
      this.URL + 'modoImpresion',
      this.requestOptions
    ).then((post) => {
      this.modoImpresion = post[0].validar;
    });
    ...
    setInterval(() => {
      ...
      if (this.registros.length >= 1) {
        if (this.registros[0].tipoCentro === 'SNTE') {
          this.activarBotonActa = this.getValues_Acta_SNTE();
        }

        if (this.registros[0].tipoCentro === 'INSTITUCION EDUCATIVA') {
          this.activarBotonActa = this.getValues_Acta_INSTITUCION_EDUCATIVA();
        }

        if (
          this.registros[0].tipoCentro === 'SUPERVISION Y JEFATURA DE SECTOR'
        ) {
          this.activarBotonActa = this.getValues_Acta_JEFATURA();
        }
        ...
        if (
          ...
          this.activarBotonActa = this.getValues_Acta_DELEGACION_REGIONAL();
        }
      }
  ...
  getValues_Acta_SNTE(): boolean {
    ...
    this.validezFormulario_Acta_SNTE[0] = horaElement.value.length > 0;

    // Validación minutos
    const minutos = document.getElementById('minutos');
    const minutosElement = minutos as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[1] = minutosElement.value.length > 0;
    ...
    this.validezFormulario_Acta_SNTE[2] =
      fechaInicioActaElement.value.length > 0;

    // Validación calle
    const calle = document.getElementById('calle');
    const calleElement = calle as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[3] = calleElement.value.length > 0;

    // Validación numcalle
    const numcalle = document.getElementById('numcalle');
    const numcalleElement = numcalle as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[4] = numcalleElement.value.length > 0;

    // Validación colonia
    const colonia = document.getElementById('colonia');
    const coloniaElement = colonia as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[5] = coloniaElement.value.length > 0;

    // Validación turno
    const turno = document.getElementById('turno');
    const turnoElement = turno as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[6] = turnoElement.value.length > 0;

    // Validación zona
    const zona = document.getElementById('zona');
    const zonaElement = zona as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[7] = zonaElement.value.length > 0;
    ...
    this.validezFormulario_Acta_SNTE[8] =
      cantidadRegitrosElement.value.length > 0;

    // Validación director
    const director = document.getElementById('director');
    const directorElement = director as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[9] = directorElement.value.length > 0;
    ...
    this.validezFormulario_Acta_SNTE[10] =
      representdocenteElement.value.length > 0;
    ...
    this.validezFormulario_Acta_SNTE[11] =
      represetsindicalElement.value.length > 0;
    ...
    this.validezFormulario_Acta_SNTE[12] = fechaFinActaElement.value.length > 0;

    // Validación hora2
    const hora2 = document.getElementById('hora2');
    const hora2Element = hora2 as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[13] = hora2Element.value.length > 0;

    // Validación colonia
    const minutos2 = document.getElementById('minutos2');
    const minutos2Element = minutos2 as HTMLInputElement;

    this.validezFormulario_Acta_SNTE[14] = minutos2Element.value.length > 0;

    if (
      this.validezFormulario_Acta_SNTE[0] == true &&
      this.validezFormulario_Acta_SNTE[1] == true &&
      this.validezFormulario_Acta_SNTE[2] == true &&
      this.validezFormulario_Acta_SNTE[3] == true &&
      this.validezFormulario_Acta_SNTE[4] == true &&
      this.validezFormulario_Acta_SNTE[5] == true &&
      this.validezFormulario_Acta_SNTE[6] == true &&
      this.validezFormulario_Acta_SNTE[7] == true &&
      this.validezFormulario_Acta_SNTE[8] == true &&
      this.validezFormulario_Acta_SNTE[9] == true &&
      this.validezFormulario_Acta_SNTE[10] == true &&
      this.validezFormulario_Acta_SNTE[11] == true &&
      this.validezFormulario_Acta_SNTE[12] == true &&
      this.validezFormulario_Acta_SNTE[13] == true &&
      this.validezFormulario_Acta_SNTE[14] == true
    ) {
      Acta = true;
    }
  ...
  getValues_Acta_INSTITUCION_EDUCATIVA(): boolean {
    let Acta = false;
    ...
    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[0] =
      horaElement.value.length > 0;

    // Validación minutos
    const minutos = document.getElementById('minutos');
    const minutosElement = minutos as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[1] =
      minutosElement.value.length > 0;

    // Validación fechaInicioActa
    const fechaInicioActa = document.getElementById('fechaInicioActa');
    const fechaInicioActaElement = fechaInicioActa as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[2] =
      fechaInicioActaElement.value.length > 0;
    ...
    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[3] =
      calleElement.value.length > 0;

    // Validación numcalle
    const numcalle = document.getElementById('numcalle');
    const numcalleElement = numcalle as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[4] =
      numcalleElement.value.length > 0;

    // Validación colonia
    const colonia = document.getElementById('colonia');
    const coloniaElement = colonia as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[5] =
      coloniaElement.value.length > 0;

    // Validación turno
    const turno = document.getElementById('turno');
    const turnoElement = turno as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[6] =
      turnoElement.value.length > 0;

    // Validación zona
    const zona = document.getElementById('zona');
    const zonaElement = zona as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[7] =
      zonaElement.value.length > 0;

    // Validación cantidadRegitros
    const cantidadRegitros = document.getElementById('cantidadRegistros');
    const cantidadRegitrosElement = cantidadRegitros as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[8] =
      cantidadRegitrosElement.value.length > 0;
    ...
    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[9] =
      directorElement.value.length > 0;

    // Validación representdocente
    const representdocente = document.getElementById('representdocente');
    const representdocenteElement = representdocente as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[10] =
      representdocenteElement.value.length > 0;
    ...
    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[11] =
      represetsindicalElement.value.length > 0;
    ...
    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[12] =
      fechaFinActaElement.value.length > 0;

    // Validación hora2
    const hora2 = document.getElementById('hora2');
    const hora2Element = hora2 as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[13] =
      hora2Element.value.length > 0;

    // Validación colonia
    const minutos2 = document.getElementById('minutos2');
    const minutos2Element = minutos2 as HTMLInputElement;

    this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[14] =
      minutos2Element.value.length > 0;

    if (
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[0] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[1] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[2] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[3] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[4] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[5] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[6] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[7] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[8] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[9] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[10] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[11] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[12] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[13] == true &&
      this.validezFormulario_Acta_INSTITUCION_EDUCATIVA[14] == true
    ) {
      Acta = true;
    }
  ...
  getValues_Acta_JEFATURA(): boolean {
    let Acta = false;
    ...
    this.validezFormulario_Acta_JEFATURA[0] = horaElement.value.length > 0;

    // Validación minutos
    const minutos = document.getElementById('minutos');
    const minutosElement = minutos as HTMLInputElement;

    this.validezFormulario_Acta_JEFATURA[1] = minutosElement.value.length > 0;

    // Validación fechaInicioActa
    const fechaInicioActa = document.getElementById('fechaInicioActa');
    const fechaInicioActaElement = fechaInicioActa as HTMLInputElement;
    this.validezFormulario_Acta_JEFATURA[2] =
      fechaInicioActaElement.value.length > 0;
    ...
    this.validezFormulario_Acta_JEFATURA[3] = calleElement.value.length > 0;

    // Validación numcalle
    const numcalle = document.getElementById('numcalle');
    const numcalleElement = numcalle as HTMLInputElement;

    this.validezFormulario_Acta_JEFATURA[4] = numcalleElement.value.length > 0;

    // Validación colonia
    const colonia = document.getElementById('colonia');
    const coloniaElement = colonia as HTMLInputElement;

    this.validezFormulario_Acta_JEFATURA[5] = coloniaElement.value.length > 0;

    // Validación turno
    const turno = document.getElementById('turno');
    const turnoElement = turno as HTMLInputElement;

    this.validezFormulario_Acta_JEFATURA[6] = turnoElement.value.length > 0;

    // Validación zona
    const zona = document.getElementById('zona');
    const zonaElement = zona as HTMLInputElement;

    this.validezFormulario_Acta_JEFATURA[7] = zonaElement.value.length > 0;

    // Validación cantidadRegitros
    const cantidadRegitros = document.getElementById('cantidadRegistros');
    const cantidadRegitrosElement = cantidadRegitros as HTMLInputElement;

    this.validezFormulario_Acta_JEFATURA[8] =
      cantidadRegitrosElement.value.length > 0;
    ...
    this.validezFormulario_Acta_JEFATURA[9] = directorElement.value.length > 0;

    // Validación represetsindical
    const represetsindical = document.getElementById('represetsindical');
    const represetsindicalElement = represetsindical as HTMLInputElement;

    this.validezFormulario_Acta_JEFATURA[10] =
      represetsindicalElement.value.length > 0;
    ...
    this.validezFormulario_Acta_JEFATURA[11] =
      fechaFinActaElement.value.length > 0;
    ...
    this.validezFormulario_Acta_JEFATURA[12] = hora2Element.value.length > 0;

    // Validación minutos2
    const minutos2 = document.getElementById('minutos2');
    const minutos2Element = minutos2 as HTMLInputElement;

    this.validezFormulario_Acta_JEFATURA[13] = minutos2Element.value.length > 0;

    if (
      this.validezFormulario_Acta_JEFATURA[0] == true &&
      this.validezFormulario_Acta_JEFATURA[1] == true &&
      this.validezFormulario_Acta_JEFATURA[2] == true &&
      this.validezFormulario_Acta_JEFATURA[3] == true &&
      this.validezFormulario_Acta_JEFATURA[4] == true &&
      this.validezFormulario_Acta_JEFATURA[5] == true &&
      this.validezFormulario_Acta_JEFATURA[6] == true &&
      this.validezFormulario_Acta_JEFATURA[7] == true &&
      this.validezFormulario_Acta_JEFATURA[8] == true &&
      this.validezFormulario_Acta_JEFATURA[9] == true &&
      this.validezFormulario_Acta_JEFATURA[10] == true &&
      this.validezFormulario_Acta_JEFATURA[11] == true &&
      this.validezFormulario_Acta_JEFATURA[12] == true &&
      this.validezFormulario_Acta_JEFATURA[13] == true
    ) {
      Acta = true;
    }
  ...
  getValues_Acta_DELEGACION_REGIONAL(): boolean {
    let Acta = false;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[0] =
      horaElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[1] =
      minutosElement.value.length > 0;

    // Validación fechaInicioActa
    const fechaInicioActa = document.getElementById('fechaInicioActa');
    const fechaInicioActaElement = fechaInicioActa as HTMLInputElement;
    this.validezFormulario_Acta_DELEGACION_REGIONAL[2] =
      fechaInicioActaElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[3] =
      calleElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[4] =
      numcalleElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[5] =
      coloniaElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[6] =
      cantidadRegitrosElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[7] =
      directorElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[8] =
      represetsindicalElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[9] =
      fechaFinActaElement.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[10] =
      hora2Element.value.length > 0;
    ...
    this.validezFormulario_Acta_DELEGACION_REGIONAL[11] =
      minutos2Element.value.length > 0;

    if (
      this.validezFormulario_Acta_DELEGACION_REGIONAL[0] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[1] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[2] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[3] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[4] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[5] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[6] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[7] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[8] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[9] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[10] == true &&
      this.validezFormulario_Acta_DELEGACION_REGIONAL[11] == true
    ) {
      Acta = true;
    }
  ...
  eliminarRegistro(registro: any): void {
    ...
    if (this.registros.length === 0) {
      ...
      this.form = {
        ...
        tipoCentro: '',
        rfc: '',
        inicioperiodo: this.form.inicioperiodo,
        finperiodo: this.form.finperiodo,
        centrotrabajo: '',
        evaluador: '',
        periodoevaluado: '',
        municipio: this.form.municipio,
    ...
    if (this.modoImpresion[0]) {
      if (this.registros[0].tipoCentro === 'INSTITUCION EDUCATIVA') {
        ...
        if (this.form.centrotrabajo.length > 49) {
          ...
          this.form.centrotrabajo.length >= 40 &&
          this.form.centrotrabajo.length <= 49
        ) {
        ...
        const existingPdfBytes = await fetch(
          '../../assets/formats/Centrales_DRs.pdf'
        ).then((res) => res.arrayBuffer());
        ...
        const palabrasEtapaTexto =
          this.datosgenerales_Backend.etapaConLetra.split(' ');
        let primeraPalabra =
          palabrasEtapaTexto.length > 1
            ? palabrasEtapaTexto.slice(0, -1).join(' ')
            : this.datosgenerales_Backend.etapaConLetra; //EXCLUIR LA PALABRA "ETAPA"
        if (palabrasEtapaTexto.length === 2) {
      ...
      this.generateAllPDF();
    } else if (this.modoImpresion[1]) {
      if (this.registros[0].tipoCentro === 'INSTITUCION EDUCATIVA') {
        const existingPdfBytes = await fetch(
          this.URL + 'files/Educativas'
        ).then((res) => res.arrayBuffer());
        ...
        console.log(this.form.municipio);
        page1.drawText(this.form.municipio, {
          x: 375,
          y: 486,
          size: 10,
        ...
        const existingPdfBytes = await fetch(this.URL + 'files/Jefatura').then(
          (res) => res.arrayBuffer()
        );
        ...
        const existingPdfBytes = await fetch(this.URL + 'files/SNTE').then(
          (res) => res.arrayBuffer()
        );
